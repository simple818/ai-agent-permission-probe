...
```bash
python3 scripts/apply_fixes.py --list                  # 列出 8 条可选修复项
python3 scripts/apply_fixes.py --fix fix-path           # 预览将要改动的内容（不做任何修改）
python3 scripts/apply_fixes.py --fix fix-path --apply   # 真正执行修复（配置文件会先自动备份）
```

修复均为可选且默认只预览。需要管理员权限的修复项（如 `/Applications`、`/usr/local`）会被检测并对普通用户跳过，同时给出 `sudo` 提示。HTML 报告底部的修复中心对每条修复都提供〔复制命令〕/〔交给 Agent 修复〕两种方式，无需接触命令行即可处理。
...
# ai-agent-permission-probe (English)
...
├── README.md                 # readme (bilingual: Chinese + English)
├── README.en.md              # standalone English readme
├── LICENSE                   # MIT
└── .gitignore
```
